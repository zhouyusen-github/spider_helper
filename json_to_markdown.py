    ...
    if value_type != str:
        ...
        try:
            ...
            try:  # 先int再float，因为float可以转化int的字符串的，反之不行
                float(value)
                answer['type'] = 'float'
                answer['preprocessing'] = 'safe_float()'
            except:
                try:
                    datetime.datetime.strptime(value, "%Y-%m-%d %H:%M:%S")
                    answer['type'] = 'datetime'
                    answer['preprocessing'] = 'safe_datetime()'
                except:
                    try:
                        datetime.datetime.strptime(value, "%Y-%m-%d")
                        answer['type'] = 'date'
                        answer['preprocessing'] = 'safe_date()'
                    except:
                        try:
                            datetime.datetime.strptime(value, "%H:%M:%S")
                            answer['type'] = 'time'
                            answer['preprocessing'] = 'safe_time()'
                        except:
                            answer['type'] = 'varchar({length})'.format(length=str(len(str(value))))
                            answer['preprocessing'] = 'safe_str()'
    return answer
...
def print_markdown(json_dict, father_key=None):  # 输出markdown表格文件
    for key in json_dict:
        ...
        if father_key:
            key = father_key + '-' + key
        preprocessing = answer['preprocessing']
        print("--- | {key} | --- | {type} | {value} | {preprocessing}".format(key=key,
                                                                              type=value_type,
                                                                              value=value,
...
print('数据库字段名 | 接口字段名 | 意义 | 数据库字段类型 | 例子 | 预处理 |异常返回| 不可为null')
print('---|---|---|---|---|---|---|---')
